...
Sender = d.read_fsm(path_script+"/abp-sender.fsm")
Receiver = d.read_fsm(path_script+"/abp-receiver.fsm")
Timer = d.read_fsm(path_script+"/timer.fsm")
BCh = d.read_fsm(path_script+"/backward-channel.fsm")
FCh = d.read_fsm(path_script+"/forward-channel.fsm")
FChn = d.read_fsm(path_script+"/forward-channel-check.fsm")
SClient = d.read_fsm(path_script+"/sending-client.fsm")
RClient = d.read_fsm(path_script+"/receiving-client.fsm")
Prop = d.read_fsm(path_script+"/prop.fsm")

controller = parallel(Sender,Receiver,Timer,BCh,SClient,RClient)
for st in controller.vs:
	st['name'] = str(st.index)

controller.Euo = controller.events.difference(FCh.Euc)
comp = parallel(FChn,controller,Prop)
d.write_fsm(path_script+"/output.fsm",comp)

print(controller.Euo)
print(controller)
weak = weakest_assumption(controller,Prop)
par = parallel(controller,weak,Prop)
d.write_fsm(path_script+"/par.fsm",par)
# normal = d.supervisor.supremal_sublanguage(controller,Prop,mode=d.supervisor.Mode.NORMAL)
# print(normal)
d.write_fsm(path_script+"/output.fsm",weak)
weak = duplicate_events(weak)
print(len(weak.vs))
comp = parallel(FCh,weak)
comp = backward_error(comp)
new_FCh = project(comp,FCh)
new_FCh = remove_duplicate_events(new_FCh)
d.write_fsm(path_script+"/new-forward-channel.fsm",new_FCh)
check = parallel(new_FCh,controller,Prop)
d.write_fsm(path_script+"/check.fsm",check)
