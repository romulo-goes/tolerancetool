...
S = d.read_fsm(path_script+"/send_channel.fsm")
A = d.read_fsm(path_script+"/ack_channel.fsm")
P = d.read_fsm(path_script+"/io_prop.fsm")
controller = d.read_fsm(path_script+"/controller2.fsm")

Unsafe = ['err']
all_edges_S = [(src.index,e,tgt.index,0) for src in S.vs for tgt in S.vs for e in S.events if (tgt.index,e) not in src['out']]
all_edges_A = [(src.index,e,tgt.index,1) for src in A.vs for tgt in A.vs for e in A.events if (tgt.index,e) not in src['out']]
all_edges = list(all_edges_S)
all_edges.extend(all_edges_A)
# print(len(all_edges))
sat = []
stack = deque()
...
while stack:
	...
	if any(set(it).issubset(set(e)) for e in sat):
		...
		# print(ctr_env.vs['name'])
		unsafe = {st.index for st in ctr_env.vs if 'err' in st['name'][0][1]}
		# print(ctr_env)
