...
M = d.read_fsm(path_script+"/controller.fsm")
N = d.read_fsm(path_script+"/send_channel_all.fsm")
# N_enh = d.read_fsm(path_script+"/channel_enh.fsm")
P = d.read_fsm(path_script+"/io_prop.fsm")
# weak = d.read_fsm(path_script+"/weak.fsm")

# comp = parallel(N_enh,M,P)
# d.write_fsm(path_script+"/output.fsm",comp)
M.Euo = M.events.difference(N.Euc)
weak = weakest_assumption(M,P)
d.write_fsm(path_script+"/weak-gen.fsm",weak)
weak = duplicate_events(weak)
comp = parallel(N,weak)
comp = backward_error(comp)
new_N = project(comp,N)
d.write_fsm(path_script+"/new_channel.fsm",new_N)
