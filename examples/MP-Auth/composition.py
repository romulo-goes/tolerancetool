...
def create_network(processes,events,messages):
	n = len(events)
	N = d.DFA()
	states = dict()
	states[""] = 0
	...
	while unvisited:
		...
		# Segmenting state knowledge
		# print(state)
		# Sending events based on receiving send message
		for p in processes:
			for msg in messages:
				next_state = list(state_list)
				if state_list == ['']:
					next_state = []
				if msg not in state:
					next_state.append(msg)
					next_state = ','.join(next_state)
					if next_state not in states:
						states[next_state] = len(states.keys())
						state_names.append(next_state)
						transitions.append((states[state],states[next_state]))
						labels.append(d.Event("s"+p+"."+msg))
						unvisited.append(next_state)
					else:
						transitions.append((states[state],states[next_state]))
						labels.append(d.Event(d.Event("s"+p+"."+msg)))
				else:
					transitions.append((states[state],states[state]))
					labels.append(d.Event(d.Event("s"+p+"."+msg)))
		
		# Receiving events based on state knowledge
		if state_list != ['']:
			for msg in state_list:
				transitions.append((states[state],states[state]))
				labels.append(d.Event(d.Event("r"+p+"."+msg)))
	N.add_vertices(
			len(states.keys()),
			state_names
		)
	N.add_edges(transitions,labels)
	# print(len(N.vs))
	return N

Human = d.read_fsm(path_script+"/Human.fsm")
Plat = d.read_fsm(path_script+"/Plat.fsm")
Server = d.read_fsm(path_script+"/Server.fsm")
Device = d.read_fsm(path_script+"/Device.fsm")

events = Human.events | Plat.events | Server.events | Device.events
print(events)
messages = ["m"+str(i) for i in range(1,2)]
# Net = create_network(["H","P","S","D"],events,messages)
# print(Net.events)
# print(Net)
M = d.composition.parallel(Plat,Server,Device)
print(len(M.vs))
for st in M.vs:
	st['name'] = str(st.index)
# M.events.update(M.events|Auth.events)
# M.Euo.update(M.Euo | Auth.Euo)
# weak = weakest_assumption(M,Auth)
