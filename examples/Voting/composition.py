...
Env = d.read_fsm(path_script+"/env_simplified.fsm")
Machine = d.read_fsm(path_script+"/machine.fsm")
# Machine = d.read_fsm(path_script+"/machine_revised.fsm")
Auth = d.read_fsm(path_script+"/property.fsm")

# M = parallel(Env,Machine)
# for st in M.vs:
# 	st['name'] = str(st.index)
# M.events.update(M.events|Auth.events)
# M.Euo.update(M.Euo | Auth.Euo)
# print(Auth.Euo)
weak = weakest_assumption(Machine,Auth)
# weak = d.read_fsm(path_script+"/weak.fsm")
# M = parallel(Env,weak)
# d.write_svg(path_script+"/M.svg",M, to_inkscape=True,)

d.write_fsm(path_script+"/output.fsm",weak)

weak = duplicate_events(weak)
# print(Env.Euc)
comp = parallel(Env,weak)
# print(comp)
comp = backward_error(comp)
d.write_fsm(path_script+"/comp.fsm",comp)
new_env = project(comp,Env)
new_env = remove_duplicate_events(new_env)
# Restriction = d.read_fsm(path_script+"/pw_restriction.fsm")
# check = parallel(new_env,Restriction)
# d.write_fsm(path_script+"/restricted_env.fsm",check)
d.write_svg(path_script+"/new_env.svg",new_env, to_inkscape=True,)
d.write_fsm(path_script+"/new_env2.fsm",new_env)

# Reduced = d.read_fsm(path_script+"/new_env_reduced.fsm")
# d.write_svg(path_script+"/new_env_reduced.svg",Reduced, to_inkscape=True,)

# check = parallel(new_env,Machine)
